...
const CourseSideBar = () => {
    ...
    return (
        <div className="bg-white w-64 ml-4 p-4 lg:block rounded-lg shadow">
            {/* Button to toggle sidebar content */}
            <button
                className="w-full flex flex-row items-center text-sm text-blue-600 border-none focus:outline-none hover:text-blue-800 transition ease-in-out duration-300"
                onClick={() => setIsMenuCollapsed((prev) => !prev)}
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className={`h-4 w-4 mr-3 transition-transform duration-300 ${
                        isMenuCollapsed ? "" : ""
                    }`}
                    viewBox="0 0 24 24"
                    fill="none"
            ...
            {!isMenuCollapsed && (
                <div>
                    {courseData.map((section, index) => (
                        <div key={section.sectionTitle} className="w-full mt-2 flex flex-col">
                            {/* Section Button */}
                            <button
                            ...
                            {openSections[index] && (
                                <div className="flex flex-col gap-2 pl-1">
                                    {section.lessons.map((lesson) => (
                                        <button
                                            key={lesson.id}
                                            className={`w-full flex items-center text-justify text-sm p-1 border-none focus:outline-none transition ease-in-out duration-300 ${
